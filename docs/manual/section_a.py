...
def section_a():
    ...
    out.append(pj(
        ...
        'managing day-to-day fleet operations. It replaces the previous '
        'combination of paper-based dispatch sheets, spreadsheets, and '
        'ad-hoc messaging applications with a single integrated platform '
    ...
    out.append(callout(
        ...
        'Scheduler application as deployed on the PythonAnywhere '
        'environment. It covers all modules visible to authenticated '
        'users. Source-code development, infrastructure administration, '
        'and external vendor (GPS, Sheets) configuration are outside its '
        'scope.',
        kind='note'))
    ...
    out.append(pj(
        'The fleet covers multiple vehicle categories — Dump Trucks (DT), '
        'Trailer Haulers (TH), Mini Dump Trucks (MDT), Self-Loading '
        'Trucks, and utility vehicles such as vans. Typical workloads '
        'include construction-site deliveries, quarry-to-batching-plant '
        'routes, and general logistics.'))
    out.append(pj(
    ...
    out.append(pj(
        ...
        'paid Developer tier. The application is reachable at an https '
        'URL and is restricted to authenticated users. The polling worker '
        'runs as an always-on task on the same host, so GPS data '
        'continues to flow even when no operator is logged into the web '
        'interface. Daily CPU budget is approximately five thousand '
        'seconds, which comfortably accommodates a sixty-second polling '
        'cadence across the full fleet.'))
    ...
    out.append(callout(
        ...
        'New user accounts are created by the IT Administrator on request '
        'from the Operations Manager. Accounts of staff leaving the '
        'organisation are deactivated within one business day of '
        'separation. See Procedure C-005 (Data Backup and User Access '
        'Management).',
        kind='note'))
