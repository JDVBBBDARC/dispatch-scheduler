...
from models import (db, TruckTypeDef, Wave, TripRecord,
...
app = Flask(__name__)
app.config['SQLALCHEMY_DATABASE_URI'] = f"sqlite:///{os.path.join(BASE_DIR, 'dispatch.db')}"
app.config['SQLALCHEMY_TRACK_MODIFICATIONS'] = False
app.config['SECRET_KEY'] = 'dispatch-scheduler-2026'
db.init_app(app)
    ...
    for key, (attr, cast) in field_map.items():
        if key in data:
            ...
            if val == '' or val is None:
                ...
                try:    setattr(trip, attr, cast(val))
                except: setattr(trip, attr, None)
...
def dashboard():
    ...
    q = (db.session.query(TripRecord).join(Wave).filter(Wave.date == d))
    if filter_truck != 'all':
    ...
    by_truck  = {}
    for tt in truck_types:
        cnt = sum(1 for t in trips if t.wave and t.wave.truck_type_id == tt.id)
...
def api_master_add(category):
    ...
    model_map = {'drivers': Driver, 'helpers': Helper,
                 'products': Product, 'clients': Client, 'dispatchers': Dispatcher}
...
def export():
    ...
    GRN    = 'C6EFCE'; YEL   = 'FFF2CC'; BLU   = 'DDEEFF'
    ...
    for tt_obj in sheets_to_create:
        ...
        title_cell       = ws['A1']
        title_cell.value = f"DISPATCH SCHEDULE — {tt_obj.name if tt_obj else 'ALL TRUCKS'} — {mo_s.strftime('%B %Y')}"
        hdr_style(title_cell, sz=13)
        ws.row_dimensions[1].height = 26

        for ci, h in enumerate(col_hdrs, 1):
            hdr_style(ws.cell(row=2, column=ci, value=h), bg='5D0E1B', sz=9)
        ws.row_dimensions[2].height = 22

        q2 = (db.session.query(TripRecord).join(Wave)
              .filter(Wave.date >= mo_s, Wave.date <= mo_e))
        if tt_obj: q2 = q2.filter(Wave.truck_type_id == tt_obj.id)
        rows = q2.order_by(Wave.date, Wave.wave_number, TripRecord.trip_number).all()

        for ri, t in enumerate(rows, 3):
            ...
            vals = [
                ...
                t.status or '', t.notes or '', t.updated_by or '',
            ]
            for ci, v in enumerate(vals, 1):
                data_style(ws.cell(row=ri, column=ci, value=v), bg=bg)
...
def attendance():
    ...
    records = (Attendance.query
               .filter(Attendance.date >= mo_s, Attendance.date <= mo_e).all())
    att_map = {(r.driver_id, r.date.day): r.status for r in records}
...
def breakdown():
    ...
    q = BreakdownLog.query.filter(
        BreakdownLog.date >= mo_s, BreakdownLog.date <= mo_e)
    if filter_status != 'all':
    ...
    all_logs = BreakdownLog.query.filter(
        BreakdownLog.date >= mo_s, BreakdownLog.date <= mo_e).all()
    summary = {s: sum(1 for l in all_logs if l.status == s) for s in BREAKDOWN_STATUSES}
    summary['Total'] = len(all_logs)
...
def api_breakdown_update(lid):
    ...
    if 'plate_id'      in data: log.plate_id    = int(data['plate_id']) if data['plate_id'] else None
    if 'date'          in data: log.date         = parse_date(data['date'])
    if 'description'   in data: log.description  = data['description'] or None
    if 'status'        in data: log.status        = data['status']
    if 'resolved_date' in data:
        rd = data['resolved_date']
        log.resolved_date = parse_date(rd) if rd else None
    if 'remarks'       in data: log.remarks       = data['remarks'] or None
...
def api_breakdown_delete(lid):
    ...
    log_change(f"Deleted breakdown #{lid}", 'breakdown')
    db.session.commit()
    return jsonify({'ok': True})
